...
Behavioral. Visual contrast lives in `DESIGN.md` (see `DESIGN.md.Colors`, `DESIGN.md.Typography`'s driver-safety contrast note, and the documented `button-ignition` contrast note).

- VoiceOver / TalkBack: every map marker announces role + state ("Meera, riding, teal marker, 3 Fun Facts"); HUD stat cards and Action Drawer rows announce label + current value/state on focus.
